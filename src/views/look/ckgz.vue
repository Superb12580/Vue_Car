  ...
  <div>
    ...
    <div style="margin: 50px">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item> Ta 的关注</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div v-if="page.total !== 0" style="margin: 50px auto">
      <el-row>
...
export default {
  ...
  methods: {
    handleSizeChange (val) {
      const that = this
      this.$http.get('/attention/item?thisId=' + that.userId + '&size=' + val).then(function (rest) {
    ...
    handleCurrentChange (val) {
      const that = this
      this.$http.get('/attention/item?thisId=' + that.userId + '&size=' + that.page.size + '&current=' + val).then(function (rest) {
        that.page = rest.data.data
      }, function (error) {
        console.log(error)
      })
  ...
  created () {
    ...
    const that = this
    this.$http.get('/attention/item?thisId=' + that.userId).then(function (rest) {
      that.page = rest.data.data
    }, function (error) {
      console.log(error)
    })
